...
class PostsOfLink extends Component {
  constructor(props) {
    ...
    this.domain = Domain + "/posts"; //server domain
  ...
  componentWillReceiveProps(nextProps) {
    //happens when actions dispatched make changes to state tree
    ...
    if (nextProps.delpostid !== this.props.delpostid) {
      let i = this.props.data.findIndex(
        post => post._id === nextProps.delpostid
      );
      if (i > -1) {
    ...
    if (this.props.data.length > 0) {
      const postItems = this.props.data.map(post => (
        <PostItem
          post={post}
          key={post._id}
          delete={post.userid === this.state.id}
        />
      ));
        ...
        <div className="PostsOfLink">
          ...
          <div className="Navigation">
            <nav className="navbar navbar-expand-lg navbar-dark bg-dark">
              <div>
                <Link to="/home"> Back to My App</Link>
              </div>
            </nav>
          </div>
      ...
      return (
        <div className="PostsOfLink">
          ...
          <div style={{ fontSize: "20px" }}> Nothing to display</div>
        </div>
      );
    } else {
      return (
